...
const WineryRegisterForm = props => {
  ...
  const registerWinery = () => {
    whineContract['registerWinery(string)'](wineryInput)
      ...
      .catch(e => {
        const message =
          e?.error?.data?.data?.message || e?.error?.message || e?.message
        console.log(e)
    ...
    <form w='100%' align='center' onSubmit={handleSubmit}>
      <FormControl isRequired>
        <FormLabel requiredIndicator='' htmlFor='winery'>
          <Heading size='sm'>Winery Registration</Heading>
        </FormLabel>
        {pending || (
          <>
            <Input
              ...
              value={wineryInput}
            />
            <FormHelperText>Add the name and click Register</FormHelperText>
          </>
        )}
      </FormControl>
      <LoadButton
